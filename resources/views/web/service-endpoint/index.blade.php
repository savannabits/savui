...
@section('title', trans('admin.service-endpoint.actions.index'))

@section('body')

    <service-endpoint-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('service-endpoints') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.service-endpoint.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('service-endpoints/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service-endpoint.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block table-responsive">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.service-endpoint.columns.id') }}</th>
                                        <th is='sortable' :column="'name'">{{ trans('admin.service-endpoint.columns.name') }}</th>
                                        <th is='sortable' :column="'endpoint'">{{ trans('admin.service-endpoint.columns.endpoint') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.service-endpoint.columns.enabled') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('savannabits/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/service-endpoints')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('savannabits/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('https://csm.strathmore.edu/mystrath/service-endpoints/bulk-destroy')">{{ trans('savannabits/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('service-endpoints/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service-endpoint.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
