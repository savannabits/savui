...
@section('title', trans('savannabits/admin-translations::admin.title'))
...
@section('body')
    ...
    <translation-listing
            ...
            :label="'{{ trans('savannabits/admin-translations::admin.index.all_groups') }}'"
            :locales="{{ $locales }}"
            inline-template >
        ...
        <div class="row">
            <div class="col">
                ...
                <modal name="edit-translation" class="modal--translation" @before-open="beforeModalOpen" v-cloak
                    ...
                    <h4 class="modal-title">{{ trans('savannabits/admin-translations::admin.index.edit') }}</h4>
                    <p class="text-center" style="word-wrap: break-word;">
                        <strong>{{ trans('savannabits/admin-translations::admin.index.default_text') }}:</strong> @{{
                        translationDefault }}</p>
                    <form @submit.prevent.once="onSubmit">
                        @foreach($locales as $locale)
                            <div class="form-group">
                                <label>{{ strtoupper($locale) }} {{ trans('savannabits/admin-translations::admin.index.translation') }}</label>
                                <input type="text" class="form-control"
                                       placeholder="{{ trans('savannabits/admin-translations::admin.index.translation_for_language', ['locale' => $locale]) }}"
                        ...
                        <div class="text-center">
                            <button class="modal-submit btn btn-block btn-primary" class="form-control" type="submit">{{ trans('savannabits/admin-ui::admin.btn.save') }} {{ trans('savannabits/admin-translations::admin.index.translation') }}</button>
                        </div>
                    </form>
                </modal>
                ...
                <modal @closed="onCloseImportModal()" name="import-translation" class="modal--translation" v-cloak
                       height="auto" :scrollable="true" :adaptive="true" :pivot-y="0.25">
                    <h4 class="modal-title">{{ trans('savannabits/admin-translations::admin.import.title') }}</h4>
                    <div class="modal-body">
                        <div v-show="currentStep == 1">
                            <form>
                                <p class="col-md-12">{{ trans('savannabits/admin-translations::admin.import.notice') }}</p>
                                <div class="row form-group col-md-12" :class="{'has-danger': errors.has('importFile')}">
                                    <div class="col-md-4 text-md-right">
                                        <label for="importFile" class="col-form-label text-md-right">{{ trans('savannabits/admin-translations::admin.import.upload_file') }}</label>
                                    </div>
                                    <div class="file-field col-md-6">
                                        <div class="btn btn-primary btn-sm col-md-12 float-left">
                                            <span><span v-if="importedFile">@{{ importedFile.name }}</span><span v-else>{{ trans('savannabits/admin-translations::admin.import.choose_file') }}</span></span>
                                ...
                                <div class="row col-md-12 form-group"
                                    ...
                                    <div class="col-md-4 text-md-right">
                                        <label for="importLanguage" class="col-form-label">{{ trans('savannabits/admin-translations::admin.import.language_to_import') }}</label>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <select class="form-control" v-model="importLanguage" name="importLanguage"
                                            ...
                                            <option value="">{{ trans('savannabits/admin-translations::admin.fields.select_language') }}</option>
                                            @foreach($locales as $locale)
                                                <div class="form-group">
                                ...
                                <div class="offset-md-4 import-checkbox">
                                    ...
                                    <label class="form-check-label" for="onlyMissingTranslations">
                                        {{ trans('savannabits/admin-translations::admin.import.do_not_override') }}
                                    </label>
                                </div>
                            </form>
                        </div>
                        <div v-show="currentStep == 2" class="col-md-12">
                            <div class="text-center col-md-12">
                                <p>{{ trans('savannabits/admin-translations::admin.import.conflict_notice_we_have_found') }}
                                    @{{ numberOfFoundTranslations }}
                                    {{ trans('savannabits/admin-translations::admin.import.conflict_notice_translations_to_be_imported') }}
                                    @{{ numberOfTranslationsToReview }}
                                    {{ trans('savannabits/admin-translations::admin.import.conflict_notice_differ') }}
                                </p>
                            </div>
                            ...
                            <table class="table table-hover">
                                <thead>
                                <tr>
                                    <th>{{ trans('savannabits/admin-translations::admin.fields.group') }}</th>
                                    <th>{{ trans('savannabits/admin-translations::admin.fields.default') }}</th>
                                    <th>{{ trans('savannabits/admin-translations::admin.fields.current_value') }}</th>
                                    <th>{{ trans('savannabits/admin-translations::admin.fields.imported_value') }}</th>
                        ...
                        <div v-show="currentStep == 3">
                            <div class="text-center col-md-12">
                                <p>
                                    @{{numberOfSuccessfullyImportedTranslations}} {{ trans('savannabits/admin-translations::admin.import.sucesfully_notice') }}
                                    @{{numberOfSuccessfullyUpdatedTranslations}} {{ trans('savannabits/admin-translations::admin.import.sucesfully_notice_update') }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <modal name="export-translation" class="modal--translation" v-cloak height="auto" :scrollable="true"
                    ...
                    <h4 class="modal-title">{{ trans('savannabits/admin-translations::admin.index.export') }}</h4>
                    <div class="text-center">
                        <form @submit.prevent.once="onSubmitExport">
                            <p class="text-left">{{ trans('savannabits/admin-translations::admin.export.notice') }}</p>
                            <div class="form-group" :class="{'has-danger': errors.has('exportLanguage')}">
                                <div class="row col-md-12">
                                    <div class="col-md-4 text-md-right">
                                        <label for="importFile" class="col-form-label text-md-right">{{ trans('savannabits/admin-translations::admin.export.language_to_export') }}</label>
                                    </div>
                                    <div class="col-md-6">
                                        <div>
                                        <button type="button" class="btn btn-secondary dropdown-toggle translations-export col-md-12 text-left" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            ...
                                            <span v-else>{{ trans('savannabits/admin-translations::admin.fields.select_language') }}</span>
                                        </button>
                            ...
                            <button class="modal-submit btn btn-block btn-primary col-md-2 float-right"
                                    class="form-control" type="submit"><i class="fa fa-file-excel-o"></i>&nbsp;{{ trans('savannabits/admin-translations::admin.btn.export') }}
                            </button>
                        </form>
                    </div>
                </modal>
                ...
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('savannabits/admin-translations::admin.index.title') }}
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="#" @click.prevent="showImport()"
                           role="button"><i
                                    class="fa fa-upload"></i>&nbsp; {{ trans('savannabits/admin-translations::admin.btn.import') }}
                        </a>
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="#" @click.prevent="showExport()"
                           role="button"><i
                                    class="fa fa-file-excel-o"></i>&nbsp; {{ trans('savannabits/admin-translations::admin.btn.export') }}
                        </a>
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="{{ url('admin/translations/rescan') }}"
                           @click.prevent="rescan('{{ url('admin/translations/rescan') }}')" role="button"><i class="fa"
                                                                                                              :class="scanning ? 'fa-spinner' : 'fa-eye'"></i>&nbsp; {{ trans('savannabits/admin-translations::admin.btn.re_scan') }}
                        </a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                                    ...
                                    <div class="col col-lg-7 col-xl-5 form-group">
                                        <div class="input-group">
                                            <div class="input-group-prepend">
                                                ...
                                                <div class="dropdown-menu">
                                                        <a class="dropdown-item" href="#"
                                                           @click.prevent="resetGroup">{{ trans('savannabits/admin-translations::admin.index.all_groups') }}</a>
                                            ...
                                            <input class="form-control"
                                                   placeholder="{{ trans('savannabits/admin-ui::admin.placeholder.search') }}"
                                                   v-model="search" @keyup.enter="filter('search', $event.target.value)"/>
                                            <span class="input-group-append">
                                                <button type="button" class="btn btn-primary"
                                                        @click="filter('search', search)"><i class="fa fa-search"></i>&nbsp; {{ trans('savannabits/admin-ui::admin.btn.search') }}</button>
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    <th is='sortable'
                                        :column="'group'">{{ trans('savannabits/admin-translations::admin.fields.group') }}</th>
                                    <th is='sortable'
                                        :column="'key'">{{ trans('savannabits/admin-translations::admin.fields.default') }}</th>
                                    <th is='sortable'
                                    ...
                                    <th is='sortable' :column="'created_at'">{{ trans('savannabits/admin-translations::admin.fields.created_at') }}</th>
                                    <th></th>
                                </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <div class="row no-gutters">
                                            <div class="col-auto">
                                                <a class="btn btn-sm btn-info" href="#"
                                                   ...
                                                   title="{{ trans('savannabits/admin-ui::admin.btn.edit') }}" role="button"><i
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <h3>{{ trans('savannabits/admin-translations::admin.index.no_items') }}</h3>
                                <p>{{ trans('savannabits/admin-translations::admin.index.try_changing_items') }}</p>
                                <a class="btn btn-primary" href="{{ url('admin/translations/rescan') }}"
                                    @click.prevent="rescan('{{ url('admin/translations/rescan') }}')" role="button"><i
                                        class="fa" :class="scanning ? 'fa-spinner' : 'fa-eye'"></i>&nbsp; {{ trans('savannabits/admin-translations::admin.btn.re_scan') }}
                                </a>
                            </div>
                        </div>
                    </div>
