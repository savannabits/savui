...
class ViewFullForm extends ViewGenerator {
     ...
     * @var string
     */
    protected $name = 'web:generate:full-form';
    ...
    public function handle()
    {
        ...
        if (!$this->route){
            if ($this->option('file-name')){
                $this->route = 'web/'.$this->fileName;
            } else {
                $this->route = 'web/'.$this->resource.'/update';
            }
        }
        ...
        $formJsPath = resource_path('js/web/'.$this->formJsRelativePath.'/Form.js');
		...
		$indexJsPath = resource_path('js/web/'.$this->formJsRelativePath.'/index.js');
